...
void mutate(int ncities){
  int indexA, indexB;
  int orderIndex;

  for(orderIndex = 0;orderIndex<populationSize;orderIndex++){
    indexA = rand() % (ncities - 1) + 1;
    indexB = rand() % (ncities - 1) + 1;  
    swap(listOfOrder[orderIndex],indexA, indexB);
  }
}
...
void pushOrder(int ncities){//inserts the order in the listOfOrder and calculate its fitness;
  int col;
  if(listOfOrder > populationSize){//reset the population if it is maximum
    listOfOrderIndex = 0;
     //save the best solution at the moment
  }
...
void printPath(int orderIndex, int ncities){// shuffle_times = if you shuffle 10 times, then you'll have 10 variations of path
  int col;
    for(col=0;col<ncities;col++){
      printf(" %d ->",listOfOrder[orderIndex][col]);
    }
    printf(" %d",listOfOrder[orderIndex][0]);
}

void shuffle(int times, int ncities){ // makes a new route starting from 1 and changes only one element
  int i,indexA, indexB;
  for(i=0;i<times;i++){
    indexA = rand() % (ncities - 1) + 1;
    indexB = rand() % (ncities - 1) + 1;
...
int main(){
  ...
  printPath(pickedOne,pointsLength);
